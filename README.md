...
The Random Forest model with balanced class weights achieved the following on the held-out test set:
...
> **Note**: The dataset is imbalanced (~85% non-responders), so accuracy alone is not the most informative metric. F1-score and recall on the positive class are more meaningful for this use case and are planned for deeper evaluation in future iterations.
